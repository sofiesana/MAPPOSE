...
from agents.mappose_agent_vect import MAPPOSE
...
class AgentFactory:
    ...
    def create_agent(agent_type: str, env, batch_size: int, lr=0.0005, discount_factor=0.99) -> Agent:
        ...
        """
        state_dim = get_full_state(env.envs[0], flatten=True).shape[0]
        print(f"State dim: {state_dim}")
        obs_dim = env.envs[0].observation_space[0].shape[0]
        print(f"Observation dim: {obs_dim}")
        num_agents = len(env.envs[0].observation_space)
        print(f"Number of agents: {num_agents}")
        num_actions = env.envs[0].action_space[0].n
